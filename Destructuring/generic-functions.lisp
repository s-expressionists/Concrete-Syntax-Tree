...
;;;; is a CST for a symbol that, when the resulting macro function is
;;;; executed on some compound form corresponding to a macro call,
;;;; will hold the remaining part of the arguments of that macro call
;;;; yet to be processed. If the lambda list is nontrivial, the LET*
;;;; bindings returned will repeatedly rebind this variable for the
;;;; sake of later parts of the lambda list.
...
(defgeneric destructuring-lambda-list-bindings
    (client lambda-list argument-variable &optional source))
...
(defgeneric parameter-groups-bindings
    (client parameter-groups argument-variable &optional source))
...
(defgeneric aux-parameter-bindings (client parameter &optional source))

;;; Return LET* bindings for a list of &AUX parameters.
(defgeneric aux-parameters-bindings (client parameters &optional source))
...
(defgeneric optional-parameter-bindings
    (client parameter argument-variable &optional source))

;;; Return LET* bindings for a list of &OPTIONAL parameters.
(defgeneric optional-parameters-bindings
    (client parameters argument-variable &optional source))
